...
requirejs.config({
  ...
  paths: {
    ...
    'services': 'services',
    'kmapping': 'libs/knockout/knockout.mapping-latest'
  },
  ...
  config: {
    ojL10n: {
      merge: {
        'ojtranslations/nls/ojtranslations': 'libs/oj/v3.0.0/resources/nls/booktranslations'
      }
    },
    text: {
 ...
 */
require(['ojs/ojcore', 'knockout', 'jquery', 'ojs/ojknockout', 'ojs/ojbutton', 'ojs/ojtoolbar','ojs/ojmenu','ojs/ojmodule','ojs/ojrouter','ojs/ojnavigationlist'], // add additional JET component modules as needed
  function(oj, ko, $) // this callback gets executed when all required modules are loaded
  {
      ...
      function HomePageViewModel(){
          ...
          self.router.configure({
              'departments': {label: 'Departments List'},
              'mathCalc': {label: 'Calculator', isDefault:true},
              'mybookslist': {label: 'Books'},
              'products': {label: 'Add Product'},
      ...
      $(document).ready(function(){
          ...
          localeLanguage = 'fr-CA'; //currently over-riding
          
          oj.Config.setLocale(localeLanguage,
            function(){
                $('html').attr('lang',localeLanguage);
            }
          );
